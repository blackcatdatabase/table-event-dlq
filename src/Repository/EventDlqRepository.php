...
namespace BlackCat\Database\Packages\EventDlq\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\EventDlq\Definitions;
use BlackCat\Database\Packages\EventDlq\Criteria;
use BlackCat\Database\Packages\EventDlq\Dto\EventDlqDto as Dto;
use BlackCat\Database\Packages\EventDlq\Mapper\EventDlqDtoMapper as RowMapper;
...
final class EventDlqRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\EventDlq\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\EventDlq\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\EventDlq\Dto\EventDlqDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\EventDlq\Dto\EventDlqDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\EventDlq\Mapper\EventDlqDtoMapper::fromRow($row);
    }
