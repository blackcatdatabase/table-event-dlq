...
final class Criteria extends BaseCriteria
{
    ...
    {
        return [ 'id', 'source', 'event_key', 'event', 'error', 'retryable', 'attempts', 'first_failed_at', 'last_failed_at' ];
    }

    /** Columns used for full-text LIKE/ILIKE searches. */
    protected function searchable(): array
    {
        return [ 'source', 'event_key', 'error' ];
    }

    /** Columns allowed in ORDER BY (falls back to filterable() when empty). */
    protected function sortable(): array
    {
        return [ 'id', 'source', 'event_key', 'error', 'retryable', 'attempts', 'first_failed_at', 'last_failed_at' ];
    }
    ...
    public static function fromDb(
        ...
        if (\method_exists(\BlackCat\Database\Packages\EventDlq\Definitions::class, 'softDeleteColumn')) {
            $soft = \BlackCat\Database\Packages\EventDlq\Definitions::softDeleteColumn();
