...
namespace BlackCat\Database\Packages\EventDlq\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\EventDlq\Dto\EventDlqDto;
use BlackCat\Database\Packages\EventDlq\Mapper\EventDlqDtoMapper;
 ...
 */
final class EventDlqAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private EventDlqRepository $eventDlqRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'event_dlq' => $this->eventDlqRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\EventDlq\ModuleException("Record {$idStr} not found for locking.");
            }
