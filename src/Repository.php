...
namespace BlackCat\Database\Packages\EventDlq;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\EventDlq\Criteria;
use BlackCat\Database\Packages\EventDlq\Repository\EventDlqRepositoryInterface;
use BlackCat\Database\Packages\EventDlq\Repository\EventDlqRepository as GeneratedRepository;
...
{
  private EventDlqRepositoryInterface $repo;
  ...
  public function __construct(private Database $db)
  {
    ...
    public function _setRepositoryForTests(RepoContract $repo): void
    {
        ...
        if (!$repo instanceof EventDlqRepositoryInterface) {
            // Allow any repo that conforms to the interface even if not generated
            throw new \InvalidArgumentException('Expected EventDlqRepositoryInterface');
        }
        $this->repo = $repo;
    }
    ...
    public function getByUnique(array $keys, bool $asDto = false): array|\BlackCat\Database\Packages\EventDlq\Dto\EventDlqDto|null { return $this->repo->getByUnique($keys, $asDto); }
    public function getById(int|string|array $id, bool $asDto = false): array|\BlackCat\Database\Packages\EventDlq\Dto\EventDlqDto|null { return $this->repo->getById($id, $asDto); }
